...
let closestValidEntryPoint (outer: Point[]) (islands: Point[][]) =
    ...
    let mutable candidate = centroid
    let step = 10.0
    let maxSearch = 400.0
    let mutable found = false

    // Spiral search outward from centroid
    for r in 0.0 .. step .. maxSearch do
        if not found then
            ...
            for i in 0 .. steps - 1 do
                if not found then
                    ...
                    if isEntryPointValid outer islands pt then
                        candidate <- pt
                        found <- true
                        ...
                        | _ -> 
                            ...
                            | None, Some offset, Some info when model.DraggingEntry ->
                                async {
                                    ...
                                    return match isEntryPointValid model.Outer model.Islands newEntry with

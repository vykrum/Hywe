...
module Geometry =
    ...
    let removeSawtooth (sqn : Sqn) (arr : (float*float)[]) : (float*float)[] =
    ...
    let polygonArea (poly: (float * float)[]) =
        match poly with
        | [||] | [|_|] | [|_; _|] -> 0.0
        | pts ->
            let n = pts.Length
    ...
    let polygonWithHolesArea (outer: (float * float)[]) (holes: (float * float)[][]) =
    ...
    let hxlPgn (sqn: Sqn) (elv: int) (vtx: (float * float)[]): Hxl[] =
        ...
        for (x2, y2) in verts do
            ...
            match lastOpt with
            | None -> lastOpt <- Some (RV(ix, iy, elv))
            | Some last ->
        ...
        for x in bMinX .. bMaxX do
            ...
            for y in bMinY .. bMaxY do
                ...
                if px < minX || px > maxX || py < minY || py > maxY then
                    ...
                    if not inside then res.Add(RV(x, y, elv))
                    else
                        ...
                        while j < islands.Length && not inIsland do
                            ...
                            if px >= iminX && px <= imaxX && py >= iminY && py <= imaxY then
                                if pointInPolygon (px, py) islands.[j] then inIsland <- true
                            j <- j + 1
                        if inIsland then res.Add(RV(x, y, elv))
        res.ToArray()
